...
def unzip_and_save_models(data, num_to_process):
    ...
    for i in range(len(data))[:num_to_process]:
        ...
        img = imread(BytesIO(img_request.content))
        ...
        zip_file = zipfile.ZipFile(BytesIO(zip_request.content))
        zip_file.extractall(save_dir+sku)

        # save example image
        imsave(save_dir+sku+'/'+sku+'.png', img)
